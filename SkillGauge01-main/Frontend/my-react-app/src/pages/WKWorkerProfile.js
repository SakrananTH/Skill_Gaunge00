...
const WorkerProfile = () => {
  ...
  const workerId = useMemo(() => {
    // Ignore mock user ID passed from dashboard fallback
    if (stateUser?.id && !String(stateUser.id).startsWith('u_mock')) {
      return stateUser.id;
    }
    try {
  ...
  const handleGoBack = () => {
    const userToPass = stateUser || {
      id: workerId,
      username: workerMeta.name,
      role: workerMeta.role
    };
    navigate('/dashboard', { state: { user: userToPass } });
  };
        ...
        <section className="wk-column">
          ...
          <form className="wk-actions" onSubmit={handleSubmit}>
            ...
            <div className="wk-actions__buttons">
              <button type="button" className="wk-btn wk-btn--ghost" onClick={handleGoBack}>ย้อนกลับ</button>
